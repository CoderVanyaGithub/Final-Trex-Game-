...
function setup() {
  ...
  trex.setCollider("rectangle",0,0,25,25);
  //trex.debug = true
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if(plantsGroup.isTouching(trex)){
      jumpSound.play();
        gameState = END;
        dieSound.play()
    }
...
function spawnPlants(){
  if (frameCount % 150 === 0) {
    var plants = createSprite(600,165,10,40);
    plants.addImage(plant);
    plants.scale = 0.10;
    plants.velocityX = -(6 + score/100);
